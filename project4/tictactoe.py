...
def load_weights_improved(policy, episode):
    """Load saved weights"""
    weights = torch.load("policy/" + str(policy.hidden_size)+"_hidden/policy-%d.pkl" % episode)
    policy.load_state_dict(weights)
...
def train_improved(policy, env, gamma=1.0, log_interval=1000, num_iter=60000):
    ...
    policy_dir = "policy/"+str(policy.hidden_size)+"_hidden/"
    average_return_dir = "average_return/"+str(policy.hidden_size)+"_hidden/"
    perf_dir =  "perf/"+str(policy.hidden_size)+"_hidden/"
